...
use crate::markdown_viewer::utils::{string_len, trim_ansi_string, wrap_cell_text, wrap_lines};
...
fn render_table<'a>(node: &'a AstNode<'a>, ctx: &mut AnsiContext) -> String {
    ...
    // centered uses source-position offset, indented uses ctx.indent(), default uses full width.
    let available_width = if ctx.center {
        let offset = node.data.borrow().sourcepos.start.column.saturating_sub(1);
        ctx.term_width.saturating_sub(offset)
    } else if ctx.should_wrap() {
        ctx.term_width.saturating_sub(ctx.indent())
    } else {
        ctx.term_width
    };
        ...
        for (col_idx, cell) in row.iter_mut().enumerate() {
            ...
            for line in cell.iter() {
                if string_len(line) > col_width {
                    new_lines.extend(wrap_cell_text(line, col_width));
                } else {
